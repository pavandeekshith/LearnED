...
const ClassroomList = () => {
  ...
  const handleAddClassroom = async () => {
    try {
      // Use the user-provided classroom ID
      const classroomId = newClassroomData.classroom_id;
          ...
          <div className="relative top-20 mx-auto p-5 border w-11/12 max-w-2xl shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* Classroom ID */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Classroom ID *</label>
                  <input
                    type="text"
                    name="classroom_id"
                    value={newClassroomData.classroom_id}
                    onChange={handleNewClassroomChange}
                    className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="e.g., MATH_10_CBSE"
                    required
                  />
                </div>
